...
var slnFile = "./Ocelot.Provider.Rafty.sln";
...
var unitTestAssemblies = @"./test/Ocelot.Provider.Rafty.UnitTests/Ocelot.Provider.Rafty.UnitTests.csproj";
var minCodeCoverage = 0d;
var coverallsRepoToken = "coveralls-repo-token-ocelot";
var coverallsRepo = "https://coveralls.io/github/ThreeMammals/Ocelot.Provider.Rafty";

// acceptance testing
var artifactsForAcceptanceTestsDir = artifactsDir + Directory("AcceptanceTests");
var acceptanceTestAssemblies = @"./test/Ocelot.Provider.Rafty.AcceptanceTests/Ocelot.Provider.Rafty.AcceptanceTests.csproj";

// integration testing
var artifactsForIntegrationTestsDir = artifactsDir + Directory("IntegrationTests");
var integrationTestAssemblies = @"./test/Ocelot.Provider.Rafty.IntegrationTests/Ocelot.Provider.Rafty.IntegrationTests.csproj";

// benchmark testing
var artifactsForBenchmarkTestsDir = artifactsDir + Directory("BenchmarkTests");
var benchmarkTestAssemblies = @"./test/Ocelot.Provider.Rafty.Benchmarks";
...
// stable releases
var tagsUrl = "https://api.github.com/repos/threemammals/ocelot.Provider.Rafty/releases/tags/";
	...
	.Does(() =>
	{
		if (IsRunningOnWindows())
		{
			...
			OpenCover(tool => 
				...
				.WithFilter("+[Ocelot.Provider.Rafty*]*")
				.WithFilter("-[xunit*]*")
				.WithFilter("-[Ocelot.Provider.Rafty*Tests]*")
			);
...
Task("CreatePackages")
	...
	.Does(() => 
	{
		...
		CopyFiles("./src/**/Ocelot.Provider.Rafty.*.nupkg", packagesDir);

		//GenerateReleaseNotes(releaseNotesFile);

        System.IO.File.WriteAllLines(artifactsFile, new[]{
            "nuget:Ocelot.Provider.Rafty." + buildVersion + ".nupkg",
